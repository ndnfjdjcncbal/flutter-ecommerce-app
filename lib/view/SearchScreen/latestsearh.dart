...
class latestsearch extends StatelessWidget {
    ...
    return GetBuilder<SearchControllerimp>(
      builder: (controller) => Expanded(
        child: Handlingdataview(
          ...
          widget: controller.latest.isEmpty
              ? _buildEmptyState()
  ...
  Widget _buildEmptyState() {
    return const Center(
      child: Column(
        ...
        children: [
          Icon(Icons.access_time, size: 70, color: Colors.grey),
          SizedBox(height: 12),
          Text(
          ...
          SizedBox(height: 5),
          Text(
            'There are no latest products available',
  ...
  Widget _buildProductCard(SearchControllerimp controller, int index) {
    final currentItem = itemsmodelall.fromJson(controller.latest[index]);
